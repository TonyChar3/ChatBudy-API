...
import {
  registerPlusSubs,
  loginUserPlanUpdate,
} from "../utils/managePlusSubscriber.js";
    ...
    const session = await stripeInstance.checkout.sessions.create({
      ...
      success_url: `${
        user_type === "new_client"
          ? `https://www.chatbudy-io.tony-char3/register?success=true`
          : user_type === "logged_client"
          ? "https://www.chatbudy-io.tony-char3/navbar/setting?success=true"
          : "https://www.chatbudy-io.tony-char3/login?success=true"
      }`,
      cancel_url: `${
        user_type === "new_client"
          ? "https://www.chatbudy-io.tony-char3/register?canceled=true"
          : user_type === "logged_client"
          ? "https://www.chatbudy-io.tony-char3/navbar/setting?canceled=true"
          : "https://www.chatbudy-io.tony-char3/login?canceled=true"
      }`,
...
const startPortalSession = asyncHandler(async (req, res, next) => {
  try {
    ...
    const session = await stripeInstance.billingPortal.sessions.create({
      ...
      return_url:
        "https://www.chatbudy-io.tony-char3/navbar/setting?portal=true",
    });
    // return the url
    res.send({ url: session.url });
  } catch (err) {
    ...
    if (data.cancel_at_period_end) {
      stripeInstance.customers
        ...
        .then((customer) => {
          ...
          const switch_plan = loginUserPlanUpdate(
