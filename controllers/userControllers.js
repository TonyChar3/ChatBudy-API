  ...
  try {
    ...
    switch (!visitor || !chatroom || !widget || !delete_user) {
      case !visitor:
        custom_statusCode = 500;
        custom_err_message =
          "Unable to delete the user visitor collection data";
        custom_err_title = "SERVER ERROR";
        break;
      case !chatroom:
        custom_statusCode = 500;
        custom_err_message =
          "Unable to delete the user chatroom collection data";
        custom_err_title = "SERVER ERROR";
        break;
      case !widget:
        custom_statusCode = 500;
...
const cleanUpNotifications = asyncHandler(async (req, res, next) => {
  try {
    ...
    const updatedNotifications = user.notification.filter(
      (notif) => !notif_array.includes(notif._id.toString())
    );
    // save it
