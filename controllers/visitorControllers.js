...
import {
  ...
  setBrowserData,
  setVisitorData,
} from "../utils/manageVisitors.js";
import {
  VerifyUserHash,
  VerifyWidgetToken,
  VerifyFirebaseToken,
} from "../middleware/authHandle.js";
...
const createVisitor = asyncHandler(async (req, res, next) => {
  try {
    ...
    // notify the user
    const notify_ws_user = sendWsUserNotification(
      "admin",
      user_hash,
      visitor_uid,
      {
        sent_from: "Admin",
        title: "New visitor",
        content: `${visitor_uid} is visiting`,
      }
    );
    ...
    await redis_widget_tokens.set(
      visitor_hash,
      JSON.stringify(generate_token.jwtToken),
      "EX",
      3600
    );
...
const deleteVisitor = asyncHandler(async (req, res, next) => {
  try {
    ...
    const visitor_index = visitor_collection.visitor.findIndex(
      (visitr) => visitr._id.toString() === visitor_id.toString()
    );
    //loop through his chatrooms to find the room
    const chatroom_index = chatroom_collection.chat_rooms.findIndex(
      (rooms) => rooms.visitor.toString() === visitor_id.toString()
    );
    if (visitor_index === -1 || chatroom_index === -1) {
      custom_statusCode = 404;
      custom_err_message = `${
        visitor_index === -1 ? "Visitor not found" : "Chatroom not found"
      }`;
      custom_err_title = "NOT FOUND";
    }
