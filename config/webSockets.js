...
import {
  saveChat,
  checkAndSetWSchatRoom,
  sendWsUserNotification,
  cacheSentChat,
  askEmailForm,
  setConversionRate,
} from "../utils/manageChatRoom.js";
...
export const webSocketServerSetUp = (redis_client, server) => {
  ...
  server.on("upgrade", async (req, socket, head) => {
    try {
      ...
      const check_for_chatroom = await checkAndSetWSchatRoom(
        "Visitor_chat",
        redis_client,
        visitorID,
        userHash
      ...
      if (!chatrooms_map.get(visitorID)) {
        chatrooms_map.set(
          check_for_chatroom.visitor_id,
          check_for_chatroom.chat_room
        );
      }
  ...
  wss.on("connection", async (ws, req) => {
    ...
    ws.on("message", async (msg, isBinary) => {
      ...
      switch (received_msg.type || received_msg.senderType) {
        case "typing":
          if (ws.id.toString() === ws_chatroom.visitor.toString()) {
            ...
            user_ws_connected.forEach((client) => {
              if (client.ws !== ws) {
                client.ws.send(
                  JSON.stringify({ type: "...", status: received_msg.status }),
                  { binary: isBinary }
                );
              }
            });
        ...
        case "set-email":
          ...
          const sanitized_value = received_msg.visitor_email.replace(
            /[^\w\s@.\-]/gi,
            ""
          );
          if (sanitized_value) {
            const set_email = await setVisitorEmail(
              userHash,
              visitorID,
              sanitized_value
            );
            ...
            const set_dummy_email = await setVisitorEmail(
              userHash,
              visitorID,
              dummy_email
            );
    ...
    ws.on("close", async () => {
      ...
      const updated_remove_ws = rogue_ws_connection.filter(
